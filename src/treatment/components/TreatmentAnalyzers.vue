...
onMounted(() => api.orchestrator.analysis.getTreatmentArtifacts(props.treatment.id, "wholesale-prices-boxplot")
...
function preconditionsMet(analyzerName: string): boolean {
    ...
    if (analyzer !== undefined) {
        return analyzer.requirements.map(r => getArtifacts(r.name))
            .filter(a => a === props.treatment.size)
...
function runAnalyzer(analyzerName: string): void {
    ...
    api.orchestrator.analysis.runTreatmentAnalyzer(props.treatment.id, analyzerName)
