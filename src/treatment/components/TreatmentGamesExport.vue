...
function addExportTask(): void {
    ...
    api.orchestrator.treatments.exportFiles(props.treatmentId, {target: target.value, baseUri: baseUri.value})
...
onMounted(() => api.orchestrator.treatments.getExportTasks(props.treatmentId)
    ...
    .catch(e => {
        console.log("unable to load existing tasks for baseline with id=" + props.treatmentId, e);
...
<template>
    <div class="border border-slate-300 rounded">
        <div>
            ...
            <div v-if="tasks.length == 0" class="text-center bg-slate-100 border border-slate-300 mx-6 py-2.5 mt-4 rounded-sm">
                No existing exports
            </div>
